...
### 17.8 MVP Agent 확장(승인 후 상태 변경 + 드라이런)
- [ ] 승인형 흐름: 추천 결과에 대해 사용자 승인 후에만 상태 변경 실행(자동 실행 금지).
- [ ] 조치 범위: 상태 변경만 수행(연기/분할/우선순위 변경은 제외).
- [ ] 드라이런 필수: 실제 업데이트 전 변경 예정 목록(대상/이전 상태/변경 상태)을 요약 표시.
- [ ] 실행 로그: 승인/실행/실패 결과를 로그로 기록(간단 요약 포함).
